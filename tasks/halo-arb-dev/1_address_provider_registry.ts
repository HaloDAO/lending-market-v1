...
task(
  'halo:arb-dev-addressproviders-1',
  'Deploy address provider, registry and fee provider for Arbitrum Rinkeby enviroment'
)
  ...
  .setAction(async ({ verify }, localBRE) => {
     ...
     */
    const addressesProvider = await deployLendingPoolAddressesProvider(HaloArbConfig.MarketId, verify);
