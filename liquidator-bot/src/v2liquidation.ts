...
import { ChainId, Token, TokenAmount, Trade } from '@uniswap/sdk';
...
export var profit_threshold = 0.1 * 10 ** 18; //in eth. A bonus below this will be ignored

export const fetchV2UnhealthyLoans = async function fetchV2UnhealthyLoans(user_id: string = '') {
  var count = 0;
  var maxCount = 6;
  var user_id_query = '';
...
function parseUsers(payload: { users: any[] }) {
  ...
  payload.users.forEach((user, i) => {
    ...
    user.collateralReserve.forEach((collateralReserve, i) => {
      ...
      totalCollateralThreshold +=
        (priceInEth * principalATokenBalance * (collateralReserve.reserve.reserveLiquidationThreshold / 10000)) /
  ...
  loans = loans.filter(
    (loan) =>
      (loan.max_borrowedPrincipal * allowedLiquidation * (loan.max_collateralBonus - 1) * loan.max_borrowedPriceInEth) /
...
async function liquidationProfit(loan) {
  ...
  var flashLoanAmountInEth =
    (flashLoanAmount * BigInt(loan.max_borrowedPriceInEth)) /
    BigInt(10 ** TOKEN_LIST[loan.max_borrowedSymbol].decimals);
  ...
  var collateralTokensFromPayout =
    (flashLoanAmountInEth_plusBonus * BigInt(10 ** TOKEN_LIST[loan.max_collateralSymbol].decimals)) /
    BigInt(loan.max_collateralPriceInEth); //this is the amount of tokens that will be received as payment for liquidation and then will need to be swapped back to token of the flashloan
  ...
  var minimumTokensAfterSwap = bestTrade
    ? (BigInt(bestTrade.outputAmount.numerator) * BigInt(10 ** TOKEN_LIST[loan.max_borrowedSymbol].decimals)) /
      BigInt(bestTrade.outputAmount.denominator)
    : BigInt(0);
  ...
  var profitInEth =
    (profitInBorrowCurrency * BigInt(loan.max_borrowedPriceInEth)) /
    BigInt(10 ** TOKEN_LIST[loan.max_borrowedSymbol].decimals);
